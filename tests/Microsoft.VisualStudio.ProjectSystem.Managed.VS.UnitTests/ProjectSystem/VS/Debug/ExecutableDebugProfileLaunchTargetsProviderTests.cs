...
{
    public class ExecutableDebugProfileLaunchTargetsProviderTests
    {
        ...
        public void GetExeAndArguments()
        {
            ...
            ExecutableDebugProfileLaunchTargetsProvider.GetExeAndArguments(false, exeIn, argsIn, out string finalExePath, out string? finalArguments);
            Assert.Equal(finalExePath, exeIn);
            Assert.Equal(finalArguments, argsIn);

            ExecutableDebugProfileLaunchTargetsProvider.GetExeAndArguments(true, exeIn, argsIn, out finalExePath, out finalArguments);
        ...
        public void GetExeAndArgumentsWithEscapedArgs()
        {
            ...
            ExecutableDebugProfileLaunchTargetsProvider.GetExeAndArguments(true, exeIn, argsInWithEscapes, out string finalExePath, out string? finalArguments);
            Assert.Equal(cmdExePath, finalExePath);
            Assert.Equal("/c \"\"c:\\foo\\bar.exe\" /foo /bar ^^ ^< ^> ^& & pause\"", finalArguments);

            ExecutableDebugProfileLaunchTargetsProvider.GetExeAndArguments(false, exeIn, argsInWithEscapes, out finalExePath, out finalArguments);
        ...
        public void GetExeAndArgumentsWithNullArgs()
        {
            ...
            ExecutableDebugProfileLaunchTargetsProvider.GetExeAndArguments(true, exeIn, null, out string finalExePath, out string? finalArguments);
            Assert.Equal(cmdExePath, finalExePath);
        ...
        {
            Assert.Equal(expected, ExecutableDebugProfileLaunchTargetsProvider.EscapeString(input, new[] { '^', '<', '>', '&' }));
        }

        private ExecutableDebugProfileLaunchTargetsProvider GetDebugTargetsProvider(string outputType = "exe", Dictionary<string, string?>? properties = null, IVsDebugger10? debugger = null, IProjectCapabilitiesScope? scope = null)
        {
        ...
        private static ExecutableDebugProfileLaunchTargetsProvider CreateInstance(
            ...
            IVsDebugger10? debugger = null)
        {
            ...
            return new ExecutableDebugProfileLaunchTargetsProvider(unconfiguredProjectVsServices, configuredProject, tokenReplacer, fileSystem, environment, activeDebugFramework, properties, threadingService, IVsUIServiceFactory.Create<SVsShellDebugger, IVsDebugger10>(debugger), remoteDebuggerAuthenticationService);
        }
    }
}
